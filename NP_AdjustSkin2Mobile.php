...
/* * * * * * * * * * * * * * * * * * * * * * *
 * NP_AdjustSkin2Mobile 0.2
 * 携帯端末 スマートフォン用Skinへ振り分けを行う
 * NP_AdjustSkin2Mobile.php - 2009/10/27 2011/1/13
...
function getName(){              return 'AdjustSkin2Mobile';}
...
}elseif(strcmp($key,"TerminalModel")==0){
	if(strcmp($this->AGENT->getModel(),$value)==0){
		return true;
	}
}else{
	return false;
...
//携帯が使用できる画面幅のROUND
function RoundWidth(){
  if($this->Display->_width && $this->Display->_width < 150){
    $this->RoundWidth = 120;
  }elseif($this->Display->_width && $this->Display->_width > 400){
    $this->RoundWidth = 480;
  }else{
    if($this->isMobile()){
      $this->RoundWidth = 240;
...
function doSkinVar(&$item,$key=null,$name=null){
  ...
  if(strcmp($key,"isMobile")==0){
    ...
    print $this->AGENT->getModel();

  }elseif(strcmp($key,"TerminalWidth")==0){
    print $this->Display->_width;

  }elseif(strcmp($key,"TerminalHeight")==0){
    print $this->Display->_height;
